...
canvas.renderAll()})};$(document).on('keyup',function(evt){if(evt.keyCode==27){if(activeShape){map4x.polygon.generatePolygon(pointArray)}
else{}}});map4x.polygon={drawPolygon:function(){polygonMode=!0;pointArray=new Array();lineArray=new Array();activeLine},addPoint:function(options){var random=Math.floor(Math.random()*(max-min+1))+min;var id=new Date().getTime()+random;var circle=new fabric.Circle({radius:8,fill:'#ffffff',stroke:'#333333',strokeWidth:0.5,left:(options.e.layerX/canvas.getZoom()),top:(options.e.layerY/canvas.getZoom()),selectable:!1,hasBorders:!1,hasControls:!1,originX:'center',originY:'center',id:id});if(pointArray.length==0){circle.set({fill:'#82c8fc'})}
...
$("#selall").on('click',function(){canvas.discardActiveObject();var sel=new fabric.ActiveSelection(canvas.getObjects(),{canvas:canvas,});canvas.setActiveObject(sel);canvas.requestRenderAll()});$("#group1").on('click',function(){if(!canvas.getActiveObject()){return}
...
#input-zoomreset:hover:after {     content: attr(data-title);
	...
	color: #323232; }
`}
function download(url,name){$('#downloader').attr({href:url,download:name})[0].click()}
function clearCanvas(canvas){canvas.clear();canvas.bg=null;canvas.frame=null;canvas.pic=null;canvas.accents=[]}
function loadJsonIntoCanvas(json,canvas,ignoreAddedEvent=!1){clearCanvas(canvas);fabric.util.enlivenObjects(json,function(objects){objects.forEach(function(obj){if(obj.id){if(obj.id==='bg'){canvas.bg=obj}else if(obj.id==='frame'){canvas.frame=obj}else if(obj.id==='pic'){canvas.pic=obj}else if(obj.id==='accent'){canvas.accents.push(obj)}}
if(ignoreAddedEvent){obj.ignoreAddedEvent=!0}
canvas.add(obj);setZIndexes(canvas);canvas.renderAll.bind(canvas)})})}
$("#canvas2json2").click(function(){var json={};json.c1=canvas.toJSON(['selectable','id']);$("#myTextArea").text(JSON.stringify(json));var a=document.createElement("a");var file=new Blob([JSON.stringify(json)],{type:'text/plain'});var url=URL.createObjectURL(file);var name='json.txt';download(url,name)});$("#loadJson2Canvas").click(function(){var dataToLoad=JSON.parse($("#myTextArea").val());canvas.loadFromJSON(dataToLoad.c1,canvas1.renderAll.bind(canvas),function(o,object){console.log(o,object)})});$('#jsonload').change(function(e){var file=e.target.files[0];if(!file)return;var reader=new FileReader();reader.onload=function(f){var data=f.target.result;data=JSON.parse(data);loadJsonIntoCanvas(data.c1.objects,canvas)};reader.readAsText(file)});$(this).val(null);$("#print").click(function(){canvas.discardActiveObject();canvas.requestRenderAll();canvas.renderAll();var dataUrl1=document.getElementById('canvas').toDataURL();var windowContent='<!DOCTYPE html>';windowContent+='<html>';windowContent+='<head><title>Print</title>';windowContent+='<style>@media print{.page-break{display:block; page-break-before:always;}}</style>';windowContent+='</head>';windowContent+='<body>';windowContent+='<img src="'+dataUrl1+'" onload=window.print();window.close();>';windowContent+='</body>';windowContent+='</html>';var printWin=window.open('','','width=340,height=260');printWin.document.open();printWin.document.write(windowContent)})